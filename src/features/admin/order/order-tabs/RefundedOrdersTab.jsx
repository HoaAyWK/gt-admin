...
const RefundedOrdersTab = ({ tableHead }) => {
  const defaultPage = 1;
  ...
  const orders = useSelector((state) => selectRefundedOrdersByPage(state, page));
  const { refundedOrdersTotalItems } = useSelector(state => state.orders);

  useEffect(() => {
    dispatch(getRefundedOrders({ page, pageSize, order, orderBy }));
  }, []);
  ...
  const handleRequestOrdersSort = (event, property) => {
    ...
    setOrder(sortOrder);
    setOrderBy(property);
    dispatch(refreshGetRefundedOrders());
    dispatch(getRefundedOrders({ page, pageSize, order: sortOrder, orderBy: property }));
  };

  const handleChangeOrdersPage = (event, newPage) => {
    setPage(newPage + 1);
    dispatch(getRefundedOrders({ page: newPage + 1, pageSize, order, orderBy }));
  };
  ...
  const handleChangeRowPerPage = (event) => {
    ...
    setPage(defaultPage);
    dispatch(getRefundedOrders({ page: defaultPage, pageSize: newPageSize, order, orderBy }));
  };
  ...
  return (
    <OrdersTab
      ...
      page={page}
      pageSize={pageSize}
      totalItems={refundedOrdersTotalItems}
      tableHead={tableHead}
...
export default RefundedOrdersTab;
